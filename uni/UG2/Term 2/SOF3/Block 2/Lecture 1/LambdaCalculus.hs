...
### Introduction
-- {useful yt video: https://www.youtube.com/watch?v=d0yEXKas8xE}
...
data LExp a = Name a  -- {variable   x}
            | Lambda a (LExp a)  -- {application   (m n)   some function m; n as a parameter of m}
            | App (LExp a) (LExp a)  -- {abstraction   λx.M   some function takes in x and does something M, which may be any of these three}
  deriving Show  -- {the interface allowing one to display}
...
pp :: Show a => LExp a -> IO ()
pp = putStrLn . pp'
  where  -- {depending on the input parameter that pp will be waiting for then passing onto pp'}
    pp' (Name v) = show v
    pp' (Lambda v e) = "(λ" ++ show v ++ " " ++ pp' e ++ ")"  -- {breaking down the expression into parts that can be recursed}
...
form of the Greek letter "beta").  For example, `(λ v (v d)) (λ w w)`  -- {(λ w w) is applied to (λ v (v d))}
is β-reduced to `(λ w w) d`, by substituting the term `(λ w w)` for  -- {replacing for all v by (λ w w)}
...
free (App f d) = free f `S.union` free d  -- {separating out the two components of the application}
free (Lambda v e) = S.delete v (free e)  -- {deleting v from the free set as it is a bound variable}
...
subst v e = sve  -- {substituting v by e}
  where  -- {upon inputting expression into subst v e (waiting for expression)}
    sve n@(Name w)     | w==v      = e
                       | otherwise = n
...
-- {I got to (λx (λw x)) - errors likely}
...
succL = Lambda 'n' (Lambda 'f' (Lambda 'x' (App (Name 'f') (App (App (Name 'n') (Name 'f')) (Name 'x')))))  -- {applies f one more time than n did}

oneL, twoL, threeL :: LExp Char
oneL   = App succL zeroL  -- {f( f(x))}
twoL   = App succL oneL  -- {f( f( f(x)))}
threeL = App succL twoL  -- {f( f( f( f(x))))}
{-
...
falseL = constL
impliesL = Lambda 'b' (Lambda 'c'  -- {the only time implication is false is when there is a true on the left and false on the right}
             (Lambda 'x' (Lambda 'y'
                (App (App (App substL (Name 'b'))
                      (App (Name 'c') (Name 'x'))) (Name 'y')))))
-- {returns x in case that b is true and c is false, y otherwise}
{-
Now we can encode the other operators and `true`:
-}
notL, trueL, orL, andL :: LExp Char
notL = Lambda 'b' (App (App impliesL (Name 'b')) falseL)  -- {encoding not b as b => False}
trueL = App notL falseL  -- {T = ¬F}
orL = Lambda 'b' (Lambda 'c'  -- {¬b => c}
                  (App (App impliesL
                        (App notL (Name 'b')))
                        (Name 'c'))) 
andL = Lambda 'b' (Lambda 'c'  -- {using de morgan's law, a /\ b == ¬(a \/ b)}
