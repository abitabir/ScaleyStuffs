...
Syntax that enables writing Haskell functions in a readable and sensible manner will be discussed, focusing on how to quickly deconstruct values, avoid long _if...then..else_ chains, 
and how to define intermediate computations for reuse. 

## Function Application {basically what we usually call function call in imperitive programming}
...
In terms of notation, one can naively describe a function application as consisting of the function name,
 followed by a space then the function argument or arguments. Where there are multiple arguments, they are separated by spaces (and not commas) and must not have parentheses around them. They need not have parentheses, unless to ensure a correct parse. Strictly speaking, Haskell functions only have a single argument, but they can return a function which is what consumes the next argument.
