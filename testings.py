    ...
    for x in range(node.state.width):
        for y in range(node.state.height):
            ...
            if me == currentPlayer:
                for (dx, dy) in [(0, +1), (+1, +1), (+1, 0), (+1, -1)]:
                    p = 1
                    while node.state.IsOnBoard(x + p * dx, y + p * dy) and node.state.board[x + p * dx][
                        y + p * dy] == me:
                        p += 1
                    n = 1
                    while node.state.IsOnBoard(x - n * dx, y - n * dy) and node.state.board[x - n * dx][
                        y - n * dy] == me:
                        n += 1
...
state_heur = Connect4State(width=3, height=3)
