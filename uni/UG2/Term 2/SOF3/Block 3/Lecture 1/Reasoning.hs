...
```haskell
∀ f, g :: a -> b {(f == g) == (∀ x::a {f x == g x})}  -- {for all xs of type a, if f(x) == g(x), the functions f and g (which take in input of type a and return output of type b) are equal}
```
...
```haskell
(rotate . rotate) (x, y)
== -- (.).0  -- {should be more specific as to which rotate is being dealt with first for machines}
rotate (rotate (x, y))
== -- rotate.0
rotate (y, x)
== -- rotate.0
(x, y)
== -- id.0
id (x, y)  -- {folding things back up (with definition going from right hand side to left hand side {spread over various lines indeedy}) rather than unfolding them lhs to rhs... ((we're proving the equalness, not tryna condense it down pers e))}
```
...
-}
bottom :: a
bottom = bottom  -- {system isn't able to detect and halt bottom as it could caput and caput'}

caput, caput' :: [a] -> a  -- {both will cause Haskell interpreater to halt upon [] input, so a failed computation (albeit a bit nicer than bottom)}
caput (x:_) = x  -- {implicit error will be raised if empty list input}
caput' []    = undefined  -- {explicitly undefined if empty list input}
caput' (x:_) = x
{-
...
== -- (.).0
rotate (rotate ⊥)
== -- rotate strict  -- {rotate is strict and so returns bottom}
rotate ⊥
== -- rotate strict
⊥
== -- id.0 or id strict
...
functions defined for `Foldable` types to define a **test** when the  -- {a successful test is not always a proof}
underlying type is in the `Eq` class.
-}
instance Foldable ProofLayout where
  foldr f z = ffz  -- foldable: f, identity: z, foldable f (z): ffz
    where
      ffz QED        = z
      ffz (p :=: pl) = f p (ffz pl)
testPL :: Eq a => ProofLayout a -> Bool
testPL QED        = True   -- {the end of an empty proof is ok}
testPL (p :=: pl) = all (==p) pl  -- {only true if all of the lines in pl are equivelant to p}
{-
...
data RGBS a b c d = RedLeaf a | GreenLeaf b  -- {inductive case sizes 0}
                  | Branch (RGBS a b c d) c (RGBS a b c d)  -- {inductive case size 2}
                  | Stem d (RGBS a b c d)  -- {inductive case size 1}
{-
...
homAppendAdd (x:xs) ys =
  ...
  (1 + length xs) + length ys
  :=:  -- {hint to add as an exercise: length.1, from rhs to lhs}
...
For the case where `xs==⊥`, we have  -- {can't risk doing any of this as an actual program XO}
```haskell
length (⊥ ++ ys)
== -- (++) is strict in its first argument  -- {fail and anything is just fail}
length ⊥
== -- length is strict
⊥
== -- (+) is strict in both arguments  -- {as in evaluates all two parameters}
⊥ + length ys  -- {may as well be XP}
== -- length is strict
length ⊥ + length ys
```
...
rv0, rv1 :: [a] -> [a] 
rv0 []     = []           -- rv0.0
rv0 (x:xs) = rv0 xs ++ [x] -- rv0.1  -- {rightfolding, so inefficent, although more easier to explain}
...
can prove it by proving that `rv1 == rv0`.  -- {justifying a complex yet effiecient computation being equivalent to one less complex yet inefficent}
