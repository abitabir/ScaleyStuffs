...
Recall that a list such as `[5, 7, 6, 2]` is actually syntactic sugar for `5:(7:(6:(2:[])))`.  A right fold {technically} replaces the
list-constructor, "cons", `(:)` by an operator and the empty list, `[]`, by a constant or the starting value. The type of `foldr` which
can easily be understood based on what is covered so far is `(a -> b -> b) -> b -> [a] -> b` in **GHC 7.8** and older. This has since
changed to `Foldable t =>  (a -> b -> b) -> b -> t a -> b` and will be discussed under *Type Classes* in Block 3. For now, just assume
the only change is from the list `[]` in `[] a` into a more generic typeclass `t` in `t a`.  It is called a right-fold because the
expression associates to the right; thus the parentheses are grouped to the right and the starting value or accumulator comes in at the
right.  The function `foldr` can be defined recursively as:
...
In general given an operator `(+)`, a starting value `x` and a list `[a, b, c]` the application `foldr (+) x [a, b, c]` result in a
single value computed by a sequence of calculations using the `(+)` operator:
...
The starting value in `revRF` is `[]` and the operator is the lambda expression `\ x xs -> xs ++ [x])`, which takes an element and a
list and concatenate the element to the end of the list. For example `revRF [2, 3, 4]` is evaluated as:

```haskell
(\ x xs -> xs ++ [x]) 2
    ((\ x xs -> xs ++ [x]) 3
        ((\ x xs -> xs ++ [x]) 4 []))  -- {4 shall be put in to [] and recurses back to put 3 in then 2}
```
-- {returns 2:3:4:[]}
...
-}
lenRF :: [a] -> Integer 
lenRF = foldr (const (1+))        0  -- {0 is the base case, returned when fold r gets to [] - cannot split anymooore}
{-

It is worth noting that `const` in the definition of `lenRF` is doing the same job as the `_` in `recLen`: both are saying that the
actual value in the list is irrelevant, only its presence is important. Thus in the definition of `lenRF`, the starting value is `0`
and the operator is `(const (1+))`. For example `lenRF [2, 3, 4]` is evaluated as:

```haskell
(const (1+)) 2
   ((const (1+)) 3
      ((const (1+)) 4 0))  -- {empty list is replaced by the initial value/base case}
~~>
(const (1+)) 2  -- {first parameter is 2, second is the accumulation}
   ((const (1+)) 3 1)
~~>
(const (1+)) 2 2  -- {what the second part (1+) becomes is where}
~~>
3
```

As a final example, based on the definition of right-folding from Block 1 exercise Q6.4; thus "a right fold replaces the list-constructor,
"cons", `(:)` by an operator and the empty list, `[]`, by a constant". The defined insertion function using a right fold `insert'` in
  Block 1 Q6.6 {in which one is to design a program to insert a value into an ordered list}, can be expanded in a similar way.

-}
insert' :: Ord a => a -> [a] -> [a]
insert' x = foldr insx [x]
  where
    insx y ys@(z:zs) | z==x && x<y = z:y:zs  -- {as is ordered, z/x (essentially the pivot) should be at the head of the list, as long as the elements being added are greater than it}
                     | otherwise   = y:ys  -- {if it isn't the head of the list anymore, elements smaller than it are to be added}
{-
...
Recall that a list such as `[5, 7, 6, 2]` is actually syntactic sugar for `5:(7:(6:(2:[])))`.  A left fold applies the operator
 on the constant or starting value and the first element of the list. The returned new accumulator value is used with another
  element of the list recursively as arguments to the operator. The type of `foldl` which again can easily be understood base
   on what is covered so far is `(a -> b -> a) -> a -> [b] -> a` in **GHC 7.8** and older. This has also changed to `Foldable
    t =>  (a -> b -> a) -> a -> t b -> b`. For now, just assume the only change is from the list `[]` in `[] b` into a more
     generic typeclass `t` in `t b`.  It is called a left-fold because the expression associates to the left; thus the
      parentheses are grouped to the left and the starting value or accumulator comes in at the left.  The function `foldl`
