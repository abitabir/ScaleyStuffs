  ...
  where
    ...
    readKO e = do
      putStrLn ("Attempt to access \"" ++ fp
                ++ "\" has raised exception: " ++ show e)  -- {show turns our exception into a message}
...
sensibleDefault fp df new = do
  ...
  where
    ...
    defaultContent _ = pure df  -- {because readFile fp returns a String wrapped up in a pure, so trying to giving them the same type of IO string}
...
prettyDefault fp df new = do
  ...
  putStrLn (either
            ((const df) :: IOException -> String) -- declaration necessary to fix exception type  -- {first arguement of either function to apply to the Left (hasn't given this function a name, just done it in line with the type declaration =0)}
            (("Existing contents of \"" ++ fp ++ "\": ") ++)  -- {second argument of either to apply to the right}
            content)
