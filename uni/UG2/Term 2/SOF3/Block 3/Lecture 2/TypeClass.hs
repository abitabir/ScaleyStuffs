...
```
Here `t` is of **kind** `* -> *`, and called a _type constructor_, rather  -- {t takes a type, and returns a type, in essence}
...
```haskell
fold  :: (Foldable t,    Monoid m          ) => t m -> m  -- {the missing operators are contained in the Monoid: operator is what is defined to be <> and base case is mempty}
foldr ::  Foldable t  => (a -> b -> b) -> b  -> t a -> b
...
-- {all Semigroup instances are (well, should be, it is the requirement of the programmer using anything as an interface to choose 
one which is applicable for their purposes) associative (the definition for <> should be) and all Monoid instances have an identity
(mempty)}
...
mconcat = foldr (<>) mempty  -- (basically what fold m does if m instance of Monoid; mconcat/<> are defined in the Monoid and Semigrooup definitions)
```
Now `mconcat ["hello", " ", "world", "!"] ~~> "hello world!"` and
`mconcat [(*2), (*3)] ~~> (*6)` without us having to specify that we  -- {<> automatically concatenates the two functions together into another function}
...
The function `all'` embeds the list of Booleans into the world of  -- {map causes every element of the list to essentially get tagged with an All}
