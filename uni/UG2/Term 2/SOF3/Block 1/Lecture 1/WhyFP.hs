...
lengthOfWeek = 7 -- number of days in a week {lengthOfWeek is 'defined to be equal to' 7 - not equal or assignment symbol, it creates a definition}
...
circumference = \r -> 2 * pi * r -- a name for the function that
                                 ...
                                 -- circle with radius r {r is input parameter}
...
same value. {no updating assignment at all}
...
function argument `g` {that returns a function that has a parameter 'a' 
which in turn returns a function which has a parameter 'b' returning
function with paramater 'c', which gives us the result of applying g to
(f applied to (a and b)) and the value of c}.
Here are example expressions using `twoop`:
`twoop` is sometimes called a **higher-order function**.
-}

exp1 = twoop (+) (*) 1 2 3 -- {~~> (*) ((+) 1 2) 3 [cuz f ->->-> g (f a b) c] ~~>} (1+2) * 3 ~~> 9
exp2 = twoop (*) (+) 1 2 3 -- {~~> (+) ((*) 1 2) 3 ~~>} 1*2 + 3   ~~> 5
...
-- {[0,2,4] returned if input n is 3; returns output [0,2] if input n is 2}
...
first_n_evens' 0 = []  -- {this case is tested first, then the case on the next line}
...
evens = [2*n | n<-[0..]] -- Data: the infinite list of elements of the
                         ...
                         -- {looks like set comprehension notation, or like list comprehension from Python but origin in Haskell - lollies thieves XOP}
first_n_evens n = take n evens -- Control is through the function
                               -- `take', found in the Prelude, a
                               -- library loaded by default.

-- {typing take into cmd terminal (after gchi command and :l filecomiling.hs) gives (Int -> [a0] -> [a0])}
...
it. {only enough of evens is evaluated so that an answer is given - not all cuz lazyyy}

-- {BUT need to remember if we type filter (<3) evens, the whole infinite list will be checked to find the next even which may be smaller than 3, so you'll need to interrupt it using Ctrl-C ~ need to be careful XOOOO potential for infinite computation}
...
programmer from a large number of errors {long before code is ever run ~ yhhh quite true}, {but} at the cost of some freedom
to the programmer {think Java with its string types c/f Python}.
...
little loss of freedom {best of both XD}.  It also has a number of optional extensions
which increase either protection or flexibility or both, including
some that {have been round long enough that they are becoming standard or}
are no longer decidable, and need heuristic, and even
human-assisted, theorem proving.

Here are the types of some of the values we have defined earlier: {we can check by :t theValueInQuestion in cmd}
-}

lengthOfWeek :: Integer
hw :: [Char] -- String is a synonym for [Char]  {of type list of characters - what a string essentially is}
(>|) :: a -> b -> a  -- {is right associative so actually (>|) :: a -> (b -> a)}
twoop :: (a -> b -> c) -> (c -> d -> e) -> a -> b -> d -> e
{-
function twoop takes a function which when given a value a, returns function of type b to c 
and a second function which when given a c, returns a function of type d and e,
where the output type of the first function in twoop is the input of the second function
i.e they are of the same types; and it takes a value of type a, b and also c,
returning the value e which is the same as the value returned in the second function
-}
evens :: (Enum c, Num c) => [c]  -- {type c is not fixed but it must have the properties
-- that it is an enumerable type AND a numeric type}
