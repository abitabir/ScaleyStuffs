...
foldl    _                z    []     = z
foldl    f                z    (x:xs) = foldl f (f x z) xs  -- {until it has not seen [], there is always a foldl at the outermost level. until we are able to get rid of the function application into a constructor application, we don't know what we're getting}
foldr :: (a -> b -> b) -> b -> [a]   -> b -- nonstrict in 3rd argument
foldr    _                z    []     = z
foldr    f                z    (x:xs) = f x (foldr f z xs)  -- {foldr can give a partial answer as f x is applied to something whcih contains foldr and if f x is able to give a partial answer, then we're game}
```
The key issue here is that evaluation of `foldl` cannot give any  -- {foldl cannae give us a result until it has seen the emoty list}
...
it can produce any output.  -- {so it is strict}
...
list, `foldl (flip (:)) []`, and suppose that we only want the first  -- {one way of reversing}
element of the result:
```haskell
head (foldl (flip (:)) [] [0..]) -- {... soooo getting the head of the flipped infinite integers list. ok}
~~>
head (foldl (flip (:)) (flip (:) [] 0) [1..])
~~>
head (foldl (flip (:)) (flip (:) (flip (:) [] 0) 1) [2..])  -- {alas, it shall never reach its destination, for it is bottom}
~~>
...
```
...
Hence we may as well evaluate `foldl` eagerly, and avoid the overheads  -- {foldl should be evaluated eagerly}
associated with lazy evaluation.

The module `Data.Foldable` contains a function `foldl'` which is an  -- {fairly standard convention if there are two versions of a function, one strict, one nonstrict, the eager version's name will be suffixed by a prime}
eager version of `foldl`.  **In practice** you should always prefer
`foldl'` to `foldl`.  Hence the best way to define `reverse` is:
-}
reverse' :: [a] -> [a]
reverse' = foldl' (flip (:)) []  -- {wellll, reverse is strict anyways so is the same as what we were using before; but still, wouldn't produce a long list of functions which have yet to be evaluated as you see in trying to get the head of [0..] reversed... it will instead evaluate as it goes along}
{-
## Adding strictness
...
-}
infixr 2 :!
data StrictList a = StrictEmpty | a :! !(StrictList a)  -- {introducing :! operator, a strict version of cons, going to be strict in its right hand argument but not left hand arg}
  deriving (Eq, Show)
...
```
Note that the spine is fully evaluated in both cases, but for `ex5`  -- {its structure is fully evaluated, but its elements may not be - they will be lazily evaluated as needed}
