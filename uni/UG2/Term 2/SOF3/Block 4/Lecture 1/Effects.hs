...
-- {$ applcationof scalar functions to scalar values; <$> application of scalar functions to structural values; <*> appplication of structural functions to structural values; >>= (bind - takes scalar then function in input ordering) bind >>= allows us to apply structures of a specific kind of function (functions from scalars to structures) to structures}
...
ex2 = Nothing <*> Just 0 -- Nothing  -- {once an error infects, there is no escape from it}
ex3 = Just (+1) <*> Nothing -- Nothing
ex4, ex5, ex6 :: Num a => [a]
ex4 = [(+1), (*2)] <*> [3, 4, 5] -- [4, 5, 6, 6, 8, 10]  -- {each function applied to each item of data}
ex5 = [] <*> [3, 4, 5] -- []
ex6 = [(+1), (*2)] <*> [] -- []
{-
...
ex10a = pure (+1) <*> (pure (*2) <*> (pure (+3) <*> [])) -- []  -- {if empty list is input, empty list shall be returned}
ex10b = [] <*> (pure (*2) <*> (pure (+3) <*> [0, 1, 2])) -- []  -- {if function applied is empty list, empty list shall be returned}
{-
...
the `do` notation.  -- {most people use do blocks instead of monad operator >>=}
-}
ex13 = do
         x <- pure 0
         return (x+1)
...
ex14 :: Num a => [a]
ex14 = do
         x <- [0, 1, 2] -- [0, 1, 2] is {already} a value in the monad {so we don't need to call pure upon them}  -- {x is drawn from this monadic structure}
         x <- pure (x+3) -- x+3 is not a value in the monad, but pure(x+3) is.  -- {we need a monadic imput for things to work smoothly}
...
ex14a = do
          ...
          let x' = x+3 -- neither x' nor x+3 are values in the monad  -- {need distinguishing variable names with let, as immutable}
...
Let's reconsider the example `ex16`.  Suppose we want to print the  -- {we have to carefully construct IO monads without mixing Monads; we have to carefully convert between different Monads}
...
At the point when our programs are outputting to the outside world we  -- {as in, () nothing (not Nothing) is returned as a standard when outputting to the output stream/printing}
...
ex16b n = do
            ...
            x <- x+1 -- Line (4)
            putStrLn ("The result of ((-'n'+3)/4)+1 is: " ++ show x)  -- {we cannot be  mixing Monads up. all the lines but this one are from the Maybe Monad. This one is from the I/O Monad.}
            return x
```
...
ex16c :: Int -> IO Int
ex16c n = do
            let (msg, val) = maybe ("Division by zero", n)  -- {what is assigned to mssg and val respectively if maybe returns Nothing}
                                   (\v->("Result is: " ++ show v, v))  -- {what is assigned to mssg and val upon input if said input is a Just value is returned by ex16 being applied to n; } 
                                   (ex16 n)  -- {returns a Maybe value, which is examined by the maybe function}
            putStr msg  -- {so we can now use upon the Maybe Monad's conversion into something which can be used by the I/O Monad}
...
ex16d n = do
            let result = ex16 n  -- {in let statements things are neutral and can exist in any Monad}
            ...
            putStrLn (", on input: " ++ show n)
            return result
-- {explcitly transferring control as we did here means we can safely mix Monads}
