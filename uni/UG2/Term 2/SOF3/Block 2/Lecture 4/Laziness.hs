...
This is called **applicative order** reduction.  -- {the strategy of reducing the leftmost innermost redex}
...
`konst 0 bottom ~~> konst 0 bottom ~~> konst 0 bottom ~~> ...`  -- {konst 0 (bottom)  -- evaluating bottom gives you bottom, which evaluated again gives you bottom again... and so on, forever}

an evaluation that continues forever.

In contrast, normal order reduction {leftmost outermost} terminates, in one step:

`konst 0 bottom ~~> 0`  -- {konst (0 bottom)  const taking the first parameter - this is lazy evaluation}

There are two important facts (provable as theorems):
1. all terminating reduction paths end in the same value  -- {i.e any evaulation paths which terminate, all reach the same terminating value [this is due to the purity of the language and not having any assignment statements]}
2. if there are any terminating paths, then normal order reduction is
   a terminating path.  -- {so using normal order reduction will give us the most amount of terminations for the programs we write}
...
fibonacci' = 1 : 1 : zipWith (+) fibonacci' (tail fibonacci')  -- {zipWith applies the input function to the zips}

{-
Simple control structures can be built by using functions such as
`take`, and `takeWhile` (See the `Prelude` or [Hoogle][2]).  More  -- {takeWhile (< 5) nats evaulates to [0, 1, 2, 3, 4]}
...
the lists are infinite.  The expression `[0..]\\[0]` is equivalent to  -- {still will return the infinite list so take n [0..] \\ [0] for some number n e.g 9 would be useful for printing in ghci}
`[1..]`, because the program only checks and removes once {on first try} for `[0]`.  However `[0]\\[1..]` is
...
turner = sieve [2..] -- an infinite list
  where sieve (p:ns) = p : sieve [q | q<-ns, q `mod` p /= 0]  -- {base case, 2 the first value is a prime number, and all of its multiples are not prime. then it recurses down the list, so that p takes on the guise of every prime number eventually and all of its muliples are deleted}
-- {a stack of sieves created, yooo. every Int needs to pass all the sieves already in the stack in order to be declared prime, and then it itself has a sieve added onto the stack with it as p, that all incoming numbers also need to pass... recursing infinitelyyy}
{-
...
-- {Eratosthenes' algorithm is less expensive than Turner's as in the latter
-- you have to divide a looot and often - by each prime you have already discovered
-- alternatively, Eratosthenes said you should go down a list of numbers,
-- crossing off any numbers which were nonprime/composite - product of the
-- number you first thought of - easy, mark off every pth element}

eratosthenes :: [Int]
eratosthenes = sieve (map Just [2..])
  where
    sieve (Just n  : ns) = n : sieve (zipWith ($) mark_n ns)  -- {if we find n to not have been checked yet, we can declare it to be prime and add it to the front of our list. we then sieve a new list: zipWith applied to mark_n applied to the tail ns; replicate replicates id n-1 times (leaves in same state), after every n-1th time i.e the nth time, const replaces any input given by the value Nothing}
      where mark_n = cycle (replicate (n-1) id ++ [const Nothing])  -- {aka after n-1, replace whatever by Nothing (at nth place). repeat repeats this whole operation forever; this is flattened into an infinite list of fucntions by concat}
-- {then we compare one function in the function list created by concat with one of the remaining numbers in ns cuzza zipWith. every number is wither paired up with id - it remains unchanged, or at every nth term the term is replaced by Nothing (whether it's a Just or a Nothing)}
    sieve (Nothing : ns) = sieve ns  -- {if sive catches something guaranteed to be composite number (denoted by Nothing as opposed to Just which hasn't been checked yet), it chucks it away and sieves the rest of the list}
-- {this works without any divisions, just repeatedly counting!}
{-
We can define many useful other values based on a list of primes.

For example, we can calculate twin prime pairs:  -- {twin prime pairs are where two adjacent prime, differ only by the value two}
-}
twinprime ::[(Int, Int)]
...
-- {maybe like: newtype Paths = Paths [] | Paths [(LeftTurn, Paths), (Ahead, Paths), (RightTurn, Paths)] | Paths [(LeftTurn, Paths), (Ahead, Paths)] | Paths [(LeftTurn, Paths), (RightTurn, Paths)] | Paths [(Ahead, Paths), (RightTurn, Paths)] | Paths [(LeftTurn, Paths)] | Paths [(Ahead, Paths)] | Paths [(RightTurn, Paths)] ... eh i dunnno}
...
compatible    (Paths vs) (m:ms) = not (null ds)
                                  && compatible (head ds) ms  -- {only evaluated if not null ds evaluated to True}
  where
    ds = [f | (n,f)<-vs, n==m]
-- {we get a semi decision procedure if ms is infinite XO, but will get decision if it is not}
{-
