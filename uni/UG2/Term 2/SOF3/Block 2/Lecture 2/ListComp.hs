...
adjpairs xs = zip xs $ tail xs  -- {$ negates the need for brackets, alternatively would have been zip xs (tail xs) to prevent zip taking the function tail as parameter}
...
-- {[a + 1..6] evaluates to [2, 3, 4, 5, 6], so up to 6 incrementing from 1 starting from the initial value; similarly [a + 2..6] evaluates to [3, 4, 5, 6]}
-}
